            ...
            var grid = (function () {
                ...
                var columns = [
                    ...
                    {render:function (cell, item) {
                        ...
                        var div = $("<div></div>").append(detail);
                        if (item.type == Consume.PAY) {
                            var cancel = $('<a class="span-margin-left20" href="#">反结算</a>');
                            cancel.click(function () {
                                ...
                                $.myAjax({
                                    type:"post",
                                    url:contextPath + "/home/cancelConsume",
                                    data:{consumeId:item.id},
                                    dataType:"json",
                                    success:function (data) {
                                        $this.removeClass("disabled");
                                        if (data[DishConst.RESULT] == DishConst.SUCCESS) {
                                            getData(grid.getPageNo());
                                        } else {
                                            alert("餐桌已经有人用餐，无法反结算。");
                                        }
                                    },
                                    error:function () {
                                        alert("操作失败，请联系管理员。");
